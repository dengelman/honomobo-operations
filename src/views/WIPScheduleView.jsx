    ...
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">{projects.filter(p => p.Stage === 'Production').length}</p>
            </div>
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">WIP Schedule by Stage</h2>
        </div>
        <div className="p-6">
          <div className="flex gap-4 overflow-x-auto pb-4">
            {projectsByStage.map(({ stage, projects: stageProjects }) => (
              <div key={stage} className="flex-shrink-0 w-64">
                <div className={`rounded-t-lg px-4 py-2 font-medium ${getStageColor(stage)}`}>{stage} ({stageProjects.length})</div>
                <div className="bg-gray-50 rounded-b-lg p-2 min-h-32 space-y-2">
                  {stageProjects.map(project => (
                    <div key={project.id} className="bg-white rounded p-3 shadow-sm border">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm">{project['Project ID']}</span>
